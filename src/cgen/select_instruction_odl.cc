...
Register *get_vreg(MModule *m, vector<MInstruction *> &res,
...
vector<MInstruction *>
select_instruction(MModule *m, Instruction &ins,
                   map<Instruction *, Register *> &instr_map,
                   map<BasicBlock *, MBasicBlock *> &bb_map,
                   map<Function *, MFunction *> &func_map,
                   map<GlobalVariable *, MachineGlobal *> &global_map,
                   Function *current_function,
                   MFunction *current_mfunction) {
...
void select_instruction(MModule *res, ANTPIE::Module *ir) {
  ...
  auto bb_map = new map<BasicBlock *, MBasicBlock *>();
  auto func_map = new map<Function *, MFunction *>();
  ...
  for (auto it = functions->begin(); it != functions->end(); ++it) {
    ...
    for (auto it = basicBlocks->begin(); it != basicBlocks->end(); ++it) {
      auto bb = *it;
      MBasicBlock *mbb = res->addBasicBlock(mfunc, bb->getName());
